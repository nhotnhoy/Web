...
Validator.isNonNegative = (myNode, message) => {
    return {
        selector: myNode,
        test: function (value) {
            const numericValue = Number(value);
            return numericValue >= 0 ? undefined : message || 'Vui lòng nhập giá trị không âm';
        }
    };
}
...
Validator.isStatus = (myNode, message) => {
    return {
        selector: myNode,
        test: function (value) {
            return value =="lock"  ? message || `Tài khoản bị khóa` : undefined;
        }
    };
}