...
const pageCount = Math.ceil(productsHtml.length / productsInPage); // Tính số lượng trang
...
function viewTopSale(inp) {
    switch (inp) {
        case 'ngay':
            listtopsale.innerHTML = '<ul class="product-list"><li class="products" ><a href="./pro_info.html"><img src="./img/aonu1.PNG" alt="product"></a><div class="products-content"><p class="price">3.900.000 đ</p><p class="description">Áo nữ </p><button class="buy">Mua ngay</button></div></li> <li class="products" ><a href="./pro_info.html"><img src="./img/aonam1.PNG" alt="product"></a><div class="products-content"><p class="price">300000 đ</p><p class="description">Áo nam năng động cá tính</p><button class="buy">Mua ngay</button></div></li></ul> '
            ren()
            break;
        case 'thang':
            listtopsale.innerHTML = '<ul class="product-list"><li class="products" ><a href="./pro_info.html"><img src="./img/aonu1.PNG" alt="product"></a><div class="products-content"><p class="price">3.900.000 đ</p><p class="description">Áo nữ </p><button class="buy">Mua ngay</button></div></li><li class="products" ><a href="./pro_info.html"><img src="./img/aonam1.PNG" alt="product"></a><div class="products-content"><p class="price">300000 đ</p><p class="description">Áo nam năng động cá tính</p><button class="buy">Mua ngay</button></div></li> </ul> '
            ...
            break;
        case 'nam':
            listtopsale.innerHTML = '<ul class="product-list"><li class="products" ><a href="./pro_info.html"><img src="./img/giaynam.PNG" alt="product"></a><div class="products-content"><p class="price">3.900.000 đ</p><p class="description">Giày thể thao GG08</p><button class="buy">Mua ngay</button></div></li><li class="products" ><a href="./pro_info.html"><img src="./img/aonu1.PNG" alt="product"></a><div class="products-content"><p class="price">99000990 đ</p><p class="description">Áo nữ</p><button class="buy">Mua ngay</button></div></li> </ul> '
            ren()
            break;
        default:
            break;
...
function ren(){
    const aElements = listtopsale.querySelectorAll('.products a');
    const buyButtons = listtopsale.querySelectorAll('.buy');

    aElements.forEach((aElement, index) => {
        aElement.addEventListener('click', (event) => {
            const pElements = listtopsale.querySelectorAll('.products-content p.description');
            pElements.forEach((pElement, ind) => {
                if (ind === index) {
                    const description = pElement.innerText;
                    const h = nonAccentVietnamese(description)
                    for (let i = 0; i < productsHtml.length; i++){
                        const f = nonAccentVietnamese(productsHtml[i].name)
    ...
    buyButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            ...
            if (loginUser) {
                ...
                if (cart.length === 0)
                    cart.push(createCartProduct(productsHtml[d]));
                else {
                    let isFind = false;
                    for (let product of cart) {
                        if (product.name == productsHtml[d].name) {
                            product.count += 1;
                            isFind = true;
                            break;
                        }
        ...
        for (let user of users) {
            if (data.email === user.email && data.password === user.password && user.typeUser === 'member') {
                ...
                if (user.status === 'lock') {
                    isFound = true;
                    $('.warning').innerHTML = `<div class="block-warning">
                 <i class="fas fa-exclamation-circle"></i> Tài khoản bị khóa
              </div>`
                    resetInput()
                }
            }
...
function renderProduct() {
    for (let item of $$('.product-pages-number')) {
        item.addEventListener('click', () => {
            df = parseInt(item.getAttribute('index'), 10); 
            $('.product-list').innerHTML = htmlProduct(df);
            render(df)
        })
    }
}
...
function render(x=0) {
    ...
    //render product
    $('.product-list').innerHTML = htmlProduct(x);
    renderProduct();

    let productBtn = $$('.products a')
    let buyBtns = $$('.products .js-buy');
